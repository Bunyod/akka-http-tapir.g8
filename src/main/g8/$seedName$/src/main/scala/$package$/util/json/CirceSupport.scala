...
// Thx to https://github.com/hseeberger/akka-http-json
...
object ErrorAccumulatingCirceSupport extends ErrorAccumulatingCirceSupport {
  ...
  final case class DecodingException(failures: NonEmptyList[DecodingFailure]) extends Exception {
...
trait FailFastUnmarshaller { this: BaseCirceSupport =>
  ...
  implicit final override def unmarshaller[A: Decoder]: FromEntityUnmarshaller[A] = {
  ...
  implicit final override def unmarshaller[A: Decoder]: FromEntityUnmarshaller[A] = {
    def decode(json: Json) =
      Decoder[A]
        ...
        .fold(failures => throw ErrorAccumulatingCirceSupport.DecodingException(failures), identity)
    jsonUnmarshaller.map(decode)
  }
